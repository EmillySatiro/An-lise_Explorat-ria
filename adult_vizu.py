...
categorical_cols = ['Sexo','Raca','Estado_Civil','Tipo_de_Emprego','Pais_de_Origem','Educacao','Profissao','Renda']
for col in categorical_cols:
    plt.figure(figsize=(8,5))
    sns.countplot(y=col, data=adult_df, order=adult_df[col].value_counts().index, palette='Set2')
...
# Faixa etária
adult_df['Faixa_Etaria'] = pd.cut(adult_df['Idade'], bins=[0,25,60,100], labels=['Jovem','Adulto','Idoso'])

# Categorizar horas de trabalho
adult_df['Horas_Categ'] = adult_df['Horas_por_Semana'].apply(lambda h: 'Part-time' if h<35 else ('Full-time' if h<=40 else 'Over-time'))
...
print("\nProporção de Renda Acima de 50K por Escolaridade:\n", adult_df.groupby('Educacao')['Renda_num'].mean().sort_values(ascending=False))
print("\nProporção de Renda Acima de 50K por Ocupação:\n", adult_df.groupby('Profissao')['Renda_num'].mean().sort_values(ascending=False))
